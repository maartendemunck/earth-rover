...
namespace earth_rover_hmi {
    ...
    HmiCommunicator::HmiCommunicator(uint8_t ce_pin, uint8_t csn_pin, CarState &car_state)
        : nrf24l01_device{ce_pin, csn_pin}, nrf24l01_power_level{1u},
          nrf24l01_fhss_channel_index{0u}, update_sequence_id{0u}, car_state{car_state} {
        ;
    }
    ...
    void HmiCommunicator::spinOnce() {
        if(channel_changed == false && since_update >= update_interval - 10u) {
            ...
            if(control_message_sent
               && update_sequence_id % 2 == 1)  // Use the odd sequence IDs for configuration.
            {
                if(!car_state.isConfigurationAvailable()) {
                    ...
                    // If the configuration should be saved, send a 'save configuration' request to
                    // the VCU.
        ...
        while(nrf24l01_device.available()) {
            ...
            switch(static_cast<ResponseMessageType>(buffer[0])) {
                ...
                case ResponseMessageType::Altitude: {
                    car_state.setAltitude(int32_t(buffer[1] | (buffer[2] << 8) | (buffer[3] << 16)
                                                  | (buffer[4] << 24)),
                                          true);
                } break;
                case ResponseMessageType::SteeringServoConfiguration: {
                    ServoConfigParams stored_configuration{
                        ...
                        uint16_t(buffer[3]
                                 | (buffer[4] << 8)),  // Pulse width to drive straight ahead.
                ...
                case ResponseMessageType::GearboxServoConfiguration: {
                    ...
                    for(unsigned index = 0; index < 2; ++index) {
                        ...
                        uint16_t pulse_width
                            = uint16_t(buffer[3 + index * 3] | (buffer[4 + index * 3] << 8));
                        if(gear >= 0 && gear <= 2) {
                    ...
                    bool complete = true;
                    for(unsigned index = 0; index <= 2; ++index) {
                        if(stored_configuration.getPulseWidth(index) == 0
                           || stored_configuration.getPulseWidth(index) == 0xffff) {
                            complete = false;
                        }
                    }
    ...
    bool HmiCommunicator::sendCurrentSteeringConfiguration() {
        static_assert(nrf24l01_payload_size >= 8, "the 'send steering configuration' message "
    ...
    bool HmiCommunicator::sendCurrentThrottleConfiguration() {
        static_assert(nrf24l01_payload_size >= 8, "the 'send throttle configuration' message "
                                                  "requires a payload size of at least 8 bytes");
        uint8_t buffer[nrf24l01_payload_size];
    ...
    bool HmiCommunicator::sendCurrentRadioConfiguration() {
        static_assert(nrf24l01_payload_size >= 8, "the 'send thradiorottle configuration' message "
                                                  "requires a payload size of at least 3 bytes");
        uint8_t buffer[nrf24l01_payload_size];
