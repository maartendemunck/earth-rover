//! Nextion HMI touch display device driver for the Earth Rover (interface and template
//! implementation).
/*!
 *  \ingroup HMI
 *  \file
...
namespace earth_rover_hmi {
    ...
    template <typename SerialDevice_t> class NextionHmiDisplay {
      private:
        ...
        SerialDevice_t
            &serial_device;  //!< Serial device used to communicate with the Nextion display.
        static constexpr uint16_t rx_buffer_size{
            8};  //!< Size of the receive buffer (can contain the largest message).
        ...
        bool configuration_pages_available;  //!< True if the configuration pages are available,
                                             //!< false if not.

        bool display_configuration_complete;  //!< Prevent processing data before configuring the
                                              //!< display.

      public:
        //! Constructor.
        /*!
         *  \param serial_device Serial device used to communicate with the Nextion HMI touch
         * display. \param car_state Car configuration and state (digital twin).
         */
        NextionHmiDisplay(SerialDevice_t &serial_device, CarState &car_state)
            : serial_device{serial_device}, rx_buffer_pointer{0u}, car_state{car_state},
              configuration_pages_available{false}, display_configuration_complete{false} {
            ;
        }
        ...
        /*!
         *  Switch the display to 115200 baud, disable command responses and switch to the
         * speedometer page.
         */
        void setup() {
        ...
        //! Spinning loop.
        /*!
         *  The spinning loop updates the display (cancelled turn signals and new sensor
         * measurements).
         */
        void spinOnce() {
      ...
      private:
        ...
        /*!
         *  \param force_update If true, update all visualizations, if false, update changed
         * visualizations.
         */
        void updateSettingsOnDisplay(bool force_update = false) {
            // Update speedometer.
            if(current_page == HmiPage::Speedometer
               && (force_update || car_state.isSpeedometerUpdated())) {
                auto speedometer = car_state.getSpeedometer();
                if(speedometer.valid == true) {
                    uint16_t angle
                        = (320u + uint16_t(26 * limit_value(speedometer.data.speed, 0., 10.)))
                          % 360u;
            ...
            if(current_page == HmiPage::Orientation
               && (force_update || car_state.isOrientationUpdated())) {
                auto orientation = car_state.getOrientation();
                if(orientation.valid == true) {
                    serial_device.printf("z_yaw.val=%d\xff\xff\xff",
                                         (90 + int16_t(orientation.data.yaw)) % 360);
                    serial_device.printf(
                        "z_pitch.val=%d\xff\xff\xff",
                        (360 + 90 + 2 * int16_t(limit_value(orientation.data.pitch, -60., 60.)))
                            % 360);
                    serial_device.printf(
                        "z_roll.val=%d\xff\xff\xff",
                        (360 + 90 + 2 * int16_t(limit_value(orientation.data.roll, -60., 60.)))
                            % 360);
                    serial_device.flush();
                }
                else {
                    ;  // TODO: Remove pointer? Don't forget to remove serial_device.flush() below
                       // if block.
                }
            }
            // Update location.
            if(current_page == HmiPage::Location
               && (force_update || car_state.isLocationUpdated())) {
                ...
                if(location.valid == true) {
                    serial_device.printf(
                        ...
                        location.data.latitude.seconds_whole,
                        location.data.latitude.seconds_frac / 100,
            ...
            // Update altitude.
            if(current_page == HmiPage::Location
               && (force_update || car_state.isAltitudeUpdated())) {
            ...
            if(current_page == HmiPage::SteeringSettings && force_update) {
                ...
                serial_device.printf("set_steering.var_channel.val=%u\xff\xff\xff",
            ...
            if(current_page == HmiPage::ThrottleSettings && force_update) {
                ...
                serial_device.printf("set_throttle.var_forward.val=%u\xff\xff\xff",
                                     config.pulse_width_maximum);
                serial_device.printf("set_throttle.var_channel.val=%u\xff\xff\xff",
                                     config.input_channel + 1);
            ...
            if(current_page == HmiPage::GearboxSettings && force_update) {
                ...
                serial_device.printf("set_gearbox.var_neutral.val=%d\xff\xff\xff",
                                     config.getPulseWidth(0));
                serial_device.printf("set_gearbox.var_low.val=%d\xff\xff\xff",
                                     config.getPulseWidth(1));
                serial_device.printf("set_gearbox.var_high.val=%d\xff\xff\xff",
                                     config.getPulseWidth(2));
                serial_device.printf("set_gearbox.var_channel.val=%d\xff\xff\xff",
                                     config.input_channel + 1);
