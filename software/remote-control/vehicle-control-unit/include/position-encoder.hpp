//! Position encoder device driver for the Earth Rover's VCU (interface and template
//! implementation).
/*!
 *  \ingroup VCU
 *  \file
...
namespace earth_rover_vcu {
     ...
     *  \ingroup VCU
     */
    template <uint8_t hall_sensor_a_pin, uint8_t hall_sensor_b_pin> class PositionEncoder {
      private:
        ISRWrapper<digitalPinToInterrupt(hall_sensor_a_pin)>
            isr_wrapper;                 //!< ISR for the first Hall effect sensor.
        ...
        static constexpr uint8_t available_intervals
            = 8;  //!< Number of between-pulse-intervals stored.
        volatile uint32_t between_pulses[available_intervals];  //!< Stored between-pulse-intervals.

        static constexpr uint16_t serialized_resolution{
            16};  //!< Resolution of serialized odometer value.
      ...
      public:
         ...
         */
        uint32_t getOdometer() const { return odometer * 1000 / pulses_per_km; }

        //! Get the current value of the trip meter in m.
        /*!
         *  \return The current value of the trip meter in m.
         */
        uint32_t getTripmeter() const { return tripmeter * 1000 / pulses_per_km; }

        //! Reset the trip meter.
        void resetTripmeter() { tripmeter = 0; }
         ...
         */
        bool saveRequired() { return (odometer - odometer_eeprom) > 16u; }

        //! Serialize the current value of the odometer.
        /*!
         *  This function uses the technique described
         * [here](https://arduino.stackexchange.com/questions/16301/) using (size - 2) counters and
         * adds a 16 bit (uint16) offset.
         *
         *  \param buffer Output buffer.
         *  \param size Size of the buffer.
         *  \return True to write a new record, false to update the existing record (or if the
