//! VCU (vehicle control unit) communicator for the Earth Rover (interface and template
//! implementation).
/*!
 *  \ingroup VCU
 *  \file
...
namespace earth_rover_vcu {
    ...
    /*!
     *  The VCU communicator handles the communication with the HMI, executing commands and
     ...
     *  \ingroup VCU
     */
    template <typename Vcu> class VcuCommunicator {
      private:
        //! Request message IDs.
        ...
        uint32_t fhss_timeout{(sizeof(nrf24l01_fhss_channels) / sizeof(nrf24l01_fhss_channels[0]))
                              * update_interval};
      ...
      public:
        ...
        VcuCommunicator(uint8_t ce_pin, uint8_t csn_pin, Vcu &vcu)
            : nrf24l01_device{ce_pin, csn_pin}, nrf24l01_fhss_channel_index{0u}, vcu{vcu} {
            ;
        }
        ...
        void spinOnce() {
            ...
            if(fhss_synced == false && since_channel_change >= fhss_timeout) {
                // If we're not synced, switch channel every 'fhss_timeout', to prevent waiting on
                // channel which is too noisy to receive anything.
                changeChannel();
            ...
            while(nrf24l01_device.available()) {
                ...
                switch(static_cast<RequestMessageType>(buffer[0])) {
                    case RequestMessageType::Control: {
                        ...
                        fhss_synced = true;
                        since_channel_change = 10u;  // Sync FHSS with sender (control message is
                                                     // sent 10ms after channel change).
                        // Send message to VCU.
                    ...
                    case RequestMessageType::RequestConfiguration: {
                        static_assert(nrf24l01_payload_size >= 2,
                                      "the 'request configuration' message requires a payload size "
                                      "of at least 2 bytes");
                    ...
                    case RequestMessageType::ConfigureSteeringServo: {
                        ...
                        vcu.configureSteeringServo(pulse_width_left, pulse_width_center,
                                                   pulse_width_right);
                    ...
                    case RequestMessageType::ConfigureEsc: {
                        ...
                        vcu.configureESC(pulse_width_backwards, pulse_width_stop,
                                         pulse_width_forward);
                    ...
                    case RequestMessageType::ConfigureRadio: {
                        ...
                        nrf24l01_device.setPALevel(
                            buffer[2]);  // Set the VCU radio PA power level directly from here.
                    } break;
      ...
      private:
        ...
        bool sendSteeringServoConfigurationMessage() {
            static_assert(nrf24l01_payload_size >= 8,
                          "the 'steering servo configuration' message requires a payload size of "
                          "at least 8 bytes");
            uint8_t buffer[nrf24l01_payload_size];
        ...
        bool sendEscConfigurationMessage() {
            static_assert(nrf24l01_payload_size >= 8,
                          "the 'ESC servo configuration' message requires a payload size of at "
                          "least 8 bytes");
            uint8_t buffer[nrf24l01_payload_size];
        ...
        bool sendGearboxServoConfigurationMessage() {
            static_assert(nrf24l01_payload_size >= 8,
                          "the 'gearbox servo configuration' message requires a payload size of at "
                          "least 9 bytes");
            uint8_t buffer[nrf24l01_payload_size];
        ...
        inline void changeChannel() {
            nrf24l01_fhss_channel_index
                = (nrf24l01_fhss_channel_index + 1)
