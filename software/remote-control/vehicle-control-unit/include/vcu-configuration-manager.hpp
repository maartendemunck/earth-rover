...
namespace earth_rover_vcu {
    ...
    /*!
     *  The configuration manager is responsible to store all configuration and calibration settings
     * in the EEPROM and implements some kind of wear leveling algorithm to enhance the life of the
     * Teensy's EEPROM. The actual serialization of configuration and calibration settings is done
     * by the subsystems.
     *
     ...
     *  \ingroup VCU
     */
    template <typename Steering_t, typename Powertrain_t, typename PositionEncoder_t,
              typename Imu_t, typename Radio_t>
    class VcuConfigurationManager {
      private:
        static constexpr uint16_t record_size{32u};  //!< Record size, including metadata.
        static constexpr uint16_t record_data_size{record_size
                                                   - 8u};  //!< Record size, excluding metadata.
        ...
        /*!
         *  \param buffer Buffer (of size record_size), last two bytes are used for the Fletcher-16
         * checksum. \return The Fletcher-16 checksum.
         */
         ...
         *  \return True if the index(location) is available, false if it is used by a configuration
         * record.
         */
        ...
        //! Check whether an object's configuration or calibration needs to be saved and save it if
        //! necessary.
        /*!
         *  \tparam Object_t Object type.
         *  \param object Object instance.
         *  \param type Record type.
         *  \param has_checksum True if the value has a checksum, false if it doesn't want a
         * checksum (e.g. odometer to lower the number of EEPROM writes). \return True if the
         * object's configuration is saved to EEPROM.
         */
        template <typename Object_t>
        bool updateStoredConfiguration(Object_t &object, RecordType type,
            ...
            if(object.saveRequired()) {
                ...
                if(used_record_indices[to_integral(type)] != 0xffff) {
                    ...
                    EEPROM.get(eeprom_offset + used_record_indices[to_integral(type)] * record_size,
                               record);
                    record_exists = true;
                }
                else {
                ...
                EEPROM.put(eeprom_offset + used_record_indices[to_integral(type)] * record_size,
                           record);
                // BEGIN DEBUG
                /*
      ...
      public:
         ...
         */
        VcuConfigurationManager(Steering_t &steering, Powertrain_t &powertrain,
                                PositionEncoder_t &position_encoder, Imu_t &imu, Radio_t &radio,
                                uint32_t eeprom_offset = 0u, uint32_t eeprom_size = 2048u)
            : eeprom_offset{eeprom_offset}, eeprom_size{eeprom_size},
              newest_record_index{UINT16_MAX}, newest_record_sequence{0u}, steering{steering},
              powertrain{powertrain}, position_encoder{position_encoder}, imu{imu}, radio{radio} {
        ...
        void setup() {
            ...
            for(uint16_t record_index = 0; record_index < record_count; ++record_index) {
                ...
                if((record_sequence != 0xffffffff && record_sequence != 0x00000000)
                   && (record_has_checksum == false
                       || fletcher16((uint8_t *)(&record)) == record_checksum)) {
                    // DEBUG
                    // Serial.printf("Parsing valid configuration record %d\n", record_index);
                    // If the record number is larger than the previous largest record number, this
                    // is a more recent message.
                    if(record_sequence > newest_record_sequence) {
                        newest_record_index = record_index;
                        newest_record_sequence = record_sequence;
                    }
                    // Parse record, store record if valid and more recent than previous cached
                    // record.
                    if(record_sequence > used_record_sequences[to_integral(record_type)]) {
                        ...
                        switch(record_type) {
                            case RecordType::Odometer: {
                                success
                                    = position_encoder.deserialize(record_data, record_data_size);
                            } break;
        ...
        //! Spinning loop.
        /*!
         *  The spinning loop makes sure all settings get saved when necessary. To prevent blocking
         * the main loop too long, maximum one configuration setting or calibration is saved during
         * each call.
         */
        void spinOnce() {
            ...
            else if(updateStoredConfiguration(steering, RecordType::SteeringServoConfiguration,
                                              true)) {
                // DEBUG
                // Serial.println("Stored steering configuration");
                ;
            }
            else if(updateStoredConfiguration(powertrain, RecordType::PowertrainConfiguration,
                                              true)) {
                // DEBUG
    ...
    template <typename Steering_t, typename Powertrain_t, typename PositionEncoder_t,
              typename Imu_t, typename Radio_t>
    auto makeVcuConfigurationManager(Steering_t &steering, Powertrain_t &powertrain,
                                     PositionEncoder_t &position_encoder, Imu_t &imu,
                                     Radio_t &radio, uint32_t eeprom_offset = 0u,
                                     uint32_t eeprom_size = 2048u) {
