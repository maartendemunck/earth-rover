//! Automotive lighting device driver for the Earth Rover's VCU (interface and inline part of the
//! implementation).
/*!
 *  \ingroup VCU
 *  \file
...
namespace earth_rover_vcu {
    ...
    class Lighting {
      ...
      private:
        ...
        static constexpr uint16_t turn_signal_period
            = 750u;  //!< Period of the turn signal (in ms).
        elapsedMillis
            since_turn_signal_flipped;   //!< Time (in ms) since the turn signals flipped state.
        bool turn_signal_flipped_state;  //!< Current state (on/off) of the turn signals or hazard
                                         //!< flashers.
      ...
      public:
        ...
        /*!
         *  The automotive lighting driver has no runtime configuration settings. This function is
         * provided for compatibility with other car device drivers.
         *
         *  \return The minimal size of the configuration block.
         */
        inline uint16_t getConfigurationSize() { return 0u; }
         ...
         * enough).
         */
        inline bool saveConfiguration(uint8_t *data, uint16_t size) { return true; }
         ...
         * checksum wrong).
         */
        inline bool loadConfiguration(uint8_t *data, uint16_t size) { return true; }
