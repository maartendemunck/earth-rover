...
namespace earth_rover_vcu {
    ...
    class Vcu {
      ...
      public:
        ...
        Vcu(Steering_t &steering, Powertrain_t &powertrain, Lighting_t &lighting,
            ...
            : steering{steering}, powertrain{powertrain}, lighting{lighting},
              position_encoder{position_encoder}, imu{imu}, gps{gps}, radio{radio} {
            ;
        }
        ...
        void setup() {
            ...
            powertrain.setup();
            // Start with stop lamps and hazard flashers on (until we receive our first command from
            // the HMI).
            lighting.setup();
            ...
            // Initialize the GPS.
            gps.setup();
            // This is a safe state, no need to call the timeout handler before we receive the first
            // control message.
         ...
         *                          (-1000 = full speed backwards ... 0 = stop ... +1000 = full
         * speed forward). \param gear Gear. \param lighting_setting Lighting state (bit 0: right
         * turn signal, bit 1: left turn signal, bit 2: dipped beam headlamps, bit 3: high beam
         * headlamps, bit 4: hazard flashers).
         */
         ...
         */
        EulerAngles_t getOrientation() { return imu.getOrientation(); }
         ...
         */
        gps_fix getGpsData() const { return gps.getCurrentGpsFix(); }
         ...
         */
        uint8_t getSteeringInputChannel() { return steering.getInputChannel(); }
         ...
         *  \return The steering input channel.
         */
        uint8_t getThrottleInputChannel() { return powertrain.getThrottleInputChannel(); }
         ...
         *  \return The steering input channel.
         */
        uint8_t getGearboxInputChannel() { return powertrain.getGearboxInputChannel(); }
         ...
         */
        uint8_t getHmiRadioPowerLevel() { return radio.getHmiRadioPowerLevel(); }

        //! Get the VCU radio's power level.
        /*!
         *  \return The VCU radio's power level.
         */
        uint8_t getVcuRadioPowerLevel() { return radio.getVcuRadioPowerLevel(); }
    ...
    template <typename Steering_t, typename Powertrain_t, typename Lighting_t,
              typename PositionEncoder_t, typename Imu_t, typename Gps_t, typename Radio_t>
    auto makeVcu(Steering_t &steering, Powertrain_t &powertrain, Lighting_t &lighting,
                 PositionEncoder_t &position_encoder, Imu_t &imu, Gps_t &gps, Radio_t &radio) {
