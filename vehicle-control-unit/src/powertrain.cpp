...
namespace earth_rover_vcu {
    ...
    Powertrain::Powertrain(uint8_t esc_pin_number, uint8_t gearbox_servo_pin_number)
        : esc_pin_number{esc_pin_number}, gearbox_servo_pin_number{gearbox_servo_pin_number},
          esc{esc_pin_number, 1000u, 1500u, 2000u, true}, gearbox_servo{gearbox_servo_pin_number,
                                                                        1000u, 1500u, 2000u, false},
          gearbox_pulse_widths{1480u, 1150u, 1800u}, throttle_input_channel{3u},
          gearbox_input_channel{2u}, input_channel_changed{false}, save_required{false},
          current_throttle_setting{0}, current_gear{0} {
        ;
    }
    ...
    void Powertrain::configureESC(uint16_t pulse_width_reverse, uint16_t pulse_width_stop,
        ...
        ConfiguredServo::Configuration configuration{pulse_width_reverse, pulse_width_stop,
                                                     pulse_width_forward, pulse_width_stop, true};
    ...
    bool Powertrain::deserialize(uint8_t *data, uint16_t size) {
        if(size >= 14u) {
            ...
            decltype(esc)::Configuration configuration{pulse_width_reverse, pulse_width_stop,
                                                       pulse_width_forward, pulse_width_stop, true};
            esc.setConfiguration(configuration);
            esc.setPosition(current_throttle_setting);
